...
const App = () => {
  ...
  const generateResponse = async (conversation, botMessageId) => {
    // Format messages for API - only include valid messages
    ...
    if (formattedMessages.length === 0) {
      setIsLoading(false);
      updateBotMessage(botMessageId, "Error: No valid message to send", true);
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        const errorMessage = data.error?.message || data.message || `HTTP ${res.status}: ${res.statusText}`;
      ...
      <main className="main-container">
        ...
        {currentConversation.messages.length === 0 ? (
          ...
          <div className="welcome-container">
            <img className="welcome-logo" src="public/gemini-chatbot-logo.svg" alt="Gemini Logo" />
